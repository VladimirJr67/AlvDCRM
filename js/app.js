...
function buildSidebar() {
  ...
  html += addItem('news', 'Новости', { badge: 'newsMenuBadge' });
  ...
  // Инструменты менеджера — только для менеджера по продажам.
  if (!admin) {
...
function injectModals() {
  ...
  const modalsHTML = `
    ...
    <div class="modal-overlay" id="newsEditorModal" onclick="if(event.target===this)closeModal('newsEditorModal')">
      <div class="modal" style="width:880px;max-width:94vw;">
        <div class="modal-head">
          <h2 id="newsEditorTitle">Новая новость</h2>
          <button type="button" class="modal-close-icon" onclick="closeModal('newsEditorModal')" title="Закрыть" aria-label="Закрыть">✕</button>
        </div>
        <input type="hidden" id="newsId">
        <div class="form-row">
          <div class="form-group"><label>Тема *</label><input type="text" id="newsTitle" placeholder="Заголовок новости"></div>
        </div>

        <div class="news-toolbar" onmousedown="if(event.target.closest('button,select'))return;event.preventDefault()">
          <select onmousedown="event.stopPropagation()" onchange="newsCmd('fontName', this.value)" title="Шрифт">
            <option value="">Шрифт</option>
            <option value="Arial, sans-serif">Arial</option>
            <option value="Georgia, serif">Georgia</option>
            <option value="'Courier New', monospace">Courier New</option>
          </select>
          <select onmousedown="event.stopPropagation()" onchange="newsCmd('formatBlock', this.value)" title="Стиль абзаца">
            <option value="">Стиль</option>
            <option value="p">Обычный текст</option>
            <option value="h3">Заголовок</option>
            <option value="blockquote">Цитата</option>
          </select>
          <button type="button" onmousedown="event.preventDefault()" onclick="newsCmd('bold')" title="Полужирный"><b>Ж</b></button>
          <button type="button" onmousedown="event.preventDefault()" onclick="newsCmd('italic')" title="Курсив"><i>К</i></button>
          <button type="button" onmousedown="event.preventDefault()" onclick="newsCmd('underline')" title="Подчёркнутый"><u>Ч</u></button>
          <select onmousedown="event.stopPropagation()" onchange="newsCmd('foreColor', this.value)" title="Цвет текста">
            <option value="">Цвет</option>
            <option value="#111111">Чёрный</option>
            <option value="#dc2626">Красный</option>
            <option value="#1a3a5c">Синий</option>
            <option value="#059669">Зелёный</option>
          </select>
          <button type="button" onmousedown="event.preventDefault()" onclick="newsCmd('insertUnorderedList')" title="Маркированный список">Список</button>
          <button type="button" onmousedown="event.preventDefault()" onclick="newsCmd('createLink')" title="Вставить ссылку">Ссылка</button>
          <button type="button" onmousedown="event.preventDefault()" onclick="document.getElementById('newsImageInput').click()" title="Вставить картинку">Картинка</button>
          <input type="file" id="newsImageInput" accept="image/*" style="display:none" onchange="newsInsertImage(event)">
        </div>

        <div id="newsBody" class="news-editor" contenteditable="true"></div>

        <div class="modal-actions">
          <button type="button" class="btn btn-secondary" onclick="closeModal('newsEditorModal')">Отмена</button>
          <button type="button" class="btn" onclick="saveNewsItem()">Опубликовать</button>
        </div>
      </div>
    </div>
        ...
        <div class="clients-bottom" id="clientsBottom">
          <div class="clients-bottom-header">
            ...
            ${isAdmin()
              ...
              : `<span class="panel-hint" title="Добавлять и изменять контактные лица может администратор">Контактные лица ведёт администратор</span>`}
          </div>
